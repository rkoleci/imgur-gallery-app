...
const GridItem = ({ title, link }: GridItemProps) => {
    ...
    return (
      <div className="rounded-sm">
          <img onClick={() => setZoom(true)} src={'https://i.imgur.com/Qu7EHDV.jpeg'} className="rounded-sm min-h-[150px] sm:min-h-[180px] lg:min-h-[300px] w-full"  />
          <div className=" bg-brown p-4 flex flex-col gap-4 rounded-sm" onClick={() => setZoom(true)}>
            ...
            <div className="flex justify-between items-center text-brown-light ">
                ...
                <p className="  text-xs leading-[18px] font-normal">85</p>
          ...
          {zoom && (
            <div onClick={() => setZoom(false)} className="scale-75 translate-x-4 skew-y-3 md:transform-none fixed top-0 right-0 w-full h-full bg-accent flex justify-center items-center">
                 <img src={'https://i.imgur.com/Qu7EHDV.jpeg'} width={300} height={300} />
              </div>
          )}
      </div>
    )
